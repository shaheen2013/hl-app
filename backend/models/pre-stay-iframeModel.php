...
function hotelBookingShareData($id)
{
    ...
    if(!$cache)
    {
        ...
        hoteles.website AS hotelUrl, hoteles.place_name, hoteles.twitterAccount, hoteles.iframe_style
        FROM hoteles 
...
function hotelBookingOfertaStay($id_hotel){
    //Get from cache
    $cacheName = 'hotelBookingOfertaStay_' . $id_hotel . '_' . $_SESSION['userLang'];
    $cache = getFromCache($cacheName);
    ...
    if(!$cache)
    {
        ...
        if($row)
        {
            $tags = array('hotel','hotel_goals','hotel_goals_'.$id_hotel, 'oferta_'.$row['id']);
            setToCache($cacheName, $row, 31536000, $tags);
        }
    }else{
        $row = $cache->get();
