...
?>
<form name="redirect" id="userAuthorizationForm" class="hotelinking-wifi-login-form" action="<?php echo (!empty($_SESSION['link-login-only']) ? $_SESSION['link-login-only'] : 'no url set') ?>" method="post">
...
<script>
    $(document).ready(function(){
        $("#hotel-room-number").change(function(){
            var roomNumber = $("#hotel-room-number").val();
            $("#userAuthorizationForm input[name=username]").val(roomNumber);
            $("#userAuthorizationForm input[name=password]").val(roomNumber);
        });
    });
</script>
