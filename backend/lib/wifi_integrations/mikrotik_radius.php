...
if (!empty($_POST) && !empty($_POST['mac'])) {
    ...
    $log->info('Mikrotik radius session info', ['session' => $_SESSION, '$_POST' => $_POST]);
}
...
$log->debug('Mikrotik radius session info', [
    'session' => $_SESSION,
    '$_POST' => $_POST,
    'form' => [
...
<form name="redirect" class="hotelinking-wifi-login-form" action="<?php echo(!empty($_SESSION['link-login-only']) ? $_SESSION['link-login-only'] : 'no url set') ?>" method="post">
    ...
    <?php if (empty($_SESSION['trial'])) { ?>
        ...
        <?php } else if (!empty(array_get($_SESSION, 'radiusTicket.username')) && !empty(array_get($_SESSION, 'radiusTicket.password'))) { ?>
            <!-- In case of radius ticket -->
            <input type="hidden" name="username" value="<?php echo(array_get($_SESSION, 'radiusTicket.username')) ?>">
            <input type="hidden" name="password" value="<?php echo(array_get($_SESSION, 'radiusTicket.password')) ?>">
        <?php } else {?>
