...
if (!empty($_POST) && !empty($_POST['mac'])) {
    ...
    $_SESSION['server-address'] = (!empty(array_get($_POST, 'server-address')) ? urldecode(array_get($_POST, 'server-address')) : '');
...
$log->info('Cyberhoteles stayapp', ['session' => $_SESSION]);
?>
...
<form name="form1" action="<?php echo !empty($_SESSION['url-login']) ? $_SESSION['url-login'] : 'http://portal.nubifi.com/MktV4Login.asp'; ?>" method="post" class="hotelinking-wifi-login-form">
    ...
    <input type="hidden" name="link-final" value="<?php echo !empty($_SESSION['portalRedirectUrl']) ? $_SESSION['portalRedirectUrl'] : (!empty($_SESSION['link-orig']) ? $_SESSION['link-orig'] : '') ?>">
