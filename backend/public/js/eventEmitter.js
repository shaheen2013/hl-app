...
HLevents = (function() {
    var tags = {};
    var hOP = tags.hasOwnProperty;

    return {
        subscribe: function(tag, listener) {
            // Create the tag's object if not yet created
            if (!hOP.call(tags, tag)) tags[tag] = [];

            // Add the listener to queue
            var index = tags[tag].push(listener) - 1;

            // Provide handle back for removal of tag
            return {
                remove: function() {
        ...
        publish: function(tag, info) {
            // If the tag doesn't exist, or there's no listeners in queue, just leave
            if (!hOP.call(tags, tag)) return;

            // Cycle through tags queue, fire!
